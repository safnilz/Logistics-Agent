...
def seed_data():
    ...
    print("Seeding jobs...")
    j1 = Job(id="JOB-001", type="Recova", client="Client A", location="Downtown", latitude=25.2048, longitude=55.2708, allocated_time="09:00 - 12:00", expected_bins=2, bin_size="240L", expected_weight_kg=400.0, assigned_vehicle="V1", status="pending", locked=False)
    j2 = Job(id="JOB-002", type="Recova", client="Client B", location="Business Bay", latitude=25.1856, longitude=55.2666, allocated_time="13:00 - 15:00", expected_bins=5, bin_size="1100L", expected_weight_kg=1200.0, assigned_vehicle="V2", status="pending", locked=False)
    j3 = Job(id="JOB-003", type="ReClaim", client="Client C", location="JLT", latitude=25.0773, longitude=55.1403, allocated_time="10:00 - 16:00", expected_bins=1, bin_size="CBM", expected_weight_kg=600.0, assigned_vehicle=None, status="unassigned", locked=False)
    db.add_all([j1, j2, j3])
    db.commit()
